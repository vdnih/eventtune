...
class OntologyMapper:
    ...
    def map_extraction(
        ...
        event_id_resolver: イベント名から既存 event_id を解決するコールバック。
            event_id が未指定（新規採番候補）のときのみ使われ、同名イベントが
            既存なら新規採番せず既存 id へ統合する（重複防止・決定論的）。
        """
        results: list[Any] = []
        transforms: list[EntityTransformation] = []
        skipped: list[SkippedRecord] = []
        ...
        for event_data in extraction.events:
            # 名前ベース照合: event_id 未指定かつ同名イベントが既存なら、その id を再利用する。
            # マルチイベント対応のため、各 event_data の名前ごとに独立して照合する。
            ...
            result = self._build_event(event_data, event_id=effective_event_id)
            if matched_name is not None:
                # Auditable AI: 新規採番せず既存イベントへ統合した根拠を記録する。
