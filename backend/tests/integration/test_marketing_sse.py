...
def fake_agent(monkeypatch):
    ...
    async def fake_chat_stream(message, session_id, space, event_id=None):
...
def test_chat_streams_sse_events(make_client, seeded_space, db, fake_agent):
    ...
    with client.stream(
        ...
        json={"message": "イベントの振り返りをして"},
    ) as res:
        assert res.status_code == 200
        assert res.headers["content-type"].startswith("text/event-stream")
        assert res.headers["x-session-id"] == "sess_test"
        events = []
    ...
    # スレッド永続化: user + assistant のスナップショットが Firestore に残る
    msgs = list(db.collection(f"spaces/{seeded_space}/threads/sess_test/messages").stream())
