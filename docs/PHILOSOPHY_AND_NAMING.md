...
#### 実装上の注意 — 実行中ステータス表示の文言はAI、配線はPython

エージェントがツール呼び出しや `run_python_code` 実行で待たせる間、マーケターに「今何をしているか」を
一言で見せたい。取り込み（ingestion）側は固定ステージ数（read/interpret/conform/...）なので
`STAGE_LABEL` という決定論の辞書1つで足りるが（`frontend/app/(app)/agent/page.tsx`）、マーケティング
チャットの `run_python_code` は毎回内容が異なる分析を行うため、ツール名ベースの固定ラベルでは
「何をしているか」が伝わらない（常に「データを分析しています」になってしまう）。

> **設計判断:** 各ツールに `intent: str` 引数を追加し、モデル自身に呼び出しごとの一言（日本語・平易）を
> 埋めさせる（`marketing_agent.py` の `_SYSTEM_PROMPT` で指示）。文言そのものの生成は意味判断（AI側）
> だが、`intent` が空だったときのフォールバック（`_TOOL_INTENT_FALLBACK`: ツール名→固定ラベル）と、
> SSEイベントへの乗せ方・フロントでの表示更新は決定論Python/TSで配線する。「意味を変えない/表示だけの
> 自由記述はAIに委ね、それ以外の制御ロジックは明文化する」という責務境界（原則4冒頭）の一例。
...
### 原則7: 個別対応は3ゲート方式で（方針→(多数のみ)セグメント→文面、AIが設計し人が承認）

個別対応の作成は、**方法決め打ち・全件フル生成のどちらもしない**。まず「どう届けるか」
（メール／架電／個別資料 等）と「誰に届けられるか」（例: メールならアドレス保有者）を検討・確認し、
そのうえで対象人数に応じて **セグメント方式**（多数向け・パターンで束ねる）と
**個別方式**（少数向け・1人ずつフル生成）のどちらを取るかを分ける。

セグメント方式は「全コンタクトに1通ずつフル生成しない」ための手段であって目的ではない
——**対象者が少ない（目安15名以下）ときはセグメント化せず、個々人に最適化する方が自然**であり、
セグメント化はカスタマイズ数・担当者の確認負荷を減らすための**例外的な最適化**と位置づける。

価値の核は、**曖昧な自然言語の意図をエージェントがマーケ知見で読み解き、ワークフロー全体を
自律的に組み立てて実行する**こと。人間に「方法を選べ→セグメントを設計せよ→分類せよ…」と
段階指示を出させない。したがってこれは人間駆動のウィザードではなく、エージェントが自分の判断で
呼ぶ**薄いツール群**（`define_segment` / `assign_segment` / `generate_patterns` / `run_assembly` /
`generate_individual_deliverables`）で構成し、オーケストレーションの知能は
**システムプロンプトのマーケ方法論**に置く（固定パイプラインとしてハードコードしない）。
...
> 束ねたものが Segment、各 Contact が L3** にあたる（個別方式では Segment を経由せず、
> Contact＝L3 の最適化を直接行う）。AIが生成してよいのは L3（動的な文脈）だけで、
> L1/L2（自社・機能の本質価値＝不変のコア）は固定する。パターン生成のガードレール
> （捏造禁止／1機能×1CEP／ブランド資産の維持）も同ドキュメントで定義し、`marketing_agent.py` の
> プロンプトに実装している（`generate_patterns` と `generate_individual_deliverables` の両方が従う）。

責務境界（原則4・「意味を変えるか否か」）に従い、各段の置き場所を分ける:

| 段 | 担い手 | 理由 |
|---|---|---|
| アプローチ方法・可否の検討 | **AI**（プロンプトの方法論） | マーケ判断＋データからの機械的な可否集計 |
| （多数時）セグメント軸の設計 | **AI**（プロンプトの方法論） | マーケ判断。固定ルール化しない |
| （多数時）バケットへの分類（`segmentation.py`） | **決定論Python**＋意味判断のみ軽量AI | 業務判断は明文化・監査可能（reason 必須） |
| コンテンツパターン／個別文面の生成 | **AI** | 文章作成はAIの中核能力。多数時はバケット数 K 回、少数時は対象者数回 |
| （多数時のみ）各メールの組み立て（プレースホルダ置換） | **決定論Python**（LLMゼロ） | 定型作業はプログラムで高速・無コスト |

**HIL（Human-In-the-Loop）は3ゲート**（①方針・可否 ②セグメント条件＋人数〔多数時のみ〕
③文面）に整理し、各ゲートは**1回の応答で確認**を取る（提案と実行宣言を分けて二重に確認を
求めない）。承認後は次のゲートまでツールを連続実行してよい——多数時は
`define_segment`＋`assign_segment` を続けて実行してからゲート②を提示し、ゲート③承認後は
`run_assembly` を追加確認なしで自動実行する。少数時（個別方式）はゲート②を経由せず、
ゲート③承認後の `generate_individual_deliverables` がそのまま確定処理となる。とくに
`run_assembly` と `generate_individual_deliverables`（全件確定）はユーザーの明示承認なしに
呼ばない。HILは固定ステートマシンではなくシステムプロンプトの指示＋会話ターンで薄く実現し、
軌道修正（方法変更・軸変更・対象絞り込み・トーン変更）は該当ゲートのやり直しで吸収する。
成果物（Segment・割り当て根拠・パターン・Deliverable）はすべて Firestore に残り Auditable。
...
> 運用負荷を高める」アンチパターンであり、本原則で置き換えた。同様に、確認ゲートを
> 過剰に細分化して1施策あたり何度も「ok」を求める運用も、本原則が避けるべき対象である。
...
│  ・個別対応: 3ゲート方式（方針→(多数のみ)軸設計→分類→パターン│
│    →組み立て／少数は個別方式で直接フル生成）                 │
...
- 個別対応は3ゲート方式（原則7）。多数向け（目安15名超）は `define_segment`（軸登録）/
  `assign_segment`（分類）/ `generate_patterns`（バケット別パターン生成）/ `run_assembly`
  （決定論的に全件組み立て、LLM不使用）。少数向け（目安15名以下）はセグメント化せず
  `generate_individual_deliverables`（対象者ごとに1通ずつフル生成）が単独で完結する。
  いずれもエージェントがHILで承認を取りつつ呼ぶ。
...
| **ラン** (Run) | `run_id` | 1回の成果物組み立て実行。多数向けは `run_assembly`（決定論的組み立て）、少数向けは `generate_individual_deliverables`（個別フル生成）が作成し即完了 |
