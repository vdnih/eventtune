...
│    save_report / define_segment / assign_segment /        │
...
Person（ハウスリストの1人）
  ...
  ├── stage: PersonStage
  │     LEAD（展示会等で出会った段階）
  │       └── engagement_level: EngagementLevel  ← 温度感
...
- オントロジー参照・操作はすべて**ツール**として提供（`list_events` / `get_event_detail` /
  ...
  `get_all_events_summary` / `get_content_catalog` / `save_report`）
...
| `DataLineage` | `IntegrationJob`（取り込みエージェントの稼働ログ）＋各レコード inline の `source_job_id`/`source_file_id` | モデル＋来歴inline化 |
