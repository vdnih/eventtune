...
> **改訂（ADR-011）**: 当初の本書は「順序非依存（`stable_id` ＋リンクスタブ）」を前提にしていたが、
> これは自然キーの正規化が完全でない限り破綻する（FK分裂・contents→event 未解決）。
> 取り込みには**依存順序**があり、マスタは観測から導出される**派生ディメンション**である、という
...
| # | ステージ | 内容 |
|---|---|---|
| 1 | **観測着地 (Land, transient)** | 全ファイルを読み、各行を行ブロック observation（`{元列:値}` の JSON object）へロスレス変換。永続しない。 |
| 2 | **解釈 (Interpret)** | observation にAIが意味づけ（エンティティ種別・列→フィールド・リンク“名”・接客事実）。schema 推論はファイル単位で1回、決定論的に各観測へ適用（行ごと LLM にはしない）。 |
| 3 | **確定 (Conform)** | 観測＋専用マスタファイルの自然キーを横断し、**実在マスタへの検索 find-or-create**（§3）で events / accounts / persons / products を確定・永続。マスタの `appeal_summary` → `appeal_vector` もここで生成。 |
| 4 | **結合 (Bind)** | `event_attendances`（接客担当/課題感/メモ付き）/ `product_interests` を、確定済みマスタの UUID へ束ねて永続。リンクスタブは作らない。 |
| 5 | **導出 (Derive)** | 各 person の `appeal_summary` / `appeal_vector` を、その人の全 `event_attendances` ＋興味製品から集約生成して書き戻す。`source_job_id` を各レコードに inline 付与し、`integration_jobs` に稼働記録を残す。 |
