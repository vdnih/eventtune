...
spaces/{space_id}/
  ...
  ※ 各 master/fact レコードは source_job_id（= job_id）を inline 保持し、
    来歴は独立コレクションではなく各データから逆引きする（ジョブ単位。ファイル単位の
    source_file_id は導入せず、ジョブの filenames で代替する）。
```
...
`source_job_id`（= batch_id）で持ち、取り込みジョブの詳細は `integration_jobs` から辿る。
...
| `backend/agents/marketing_agent.py` | ツール（appeal_vector の意味検索 `find_relevant_for_person`、実装済み）と `_SYSTEM_PROMPT` のオントロジー記述 |
| `backend/segmentation.py` | バケット代表ベクトルと appeal_vector のコサイン近接を一次候補にした分類（意味検索の消費側） |
| `backend/routers/data.py` | 閲覧ビューの `VIEWS` レジストリ |
| フロント `app/(app)/explorer/page.tsx` ＋ `components/ui/{DataTable,Drawer,format}.tsx` | 汎用データブラウザ（モデル非依存） |
