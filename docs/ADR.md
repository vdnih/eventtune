...
- **追記（2026-07-10・その2）**: 決定7の PDF 拒否方針を改める。PDF・PowerPoint (.pptx) をテキスト抽出限定（`readers.read_pdf` / `readers.read_pptx`。pypdf / python-pptx によるページ・スライド単位のテキスト連結。docx と同型で Read ステージ止まり）で受理するよう変更した。表のレイアウト・図解・画像内の文字が欠落し得るリスクは、黙って通さず `FilePlan.extraction_caveat`（拡張子から一意に決まる P1 の決定論的な定型文。AI は関与しない）として Confirm 画面に必ず表示することで可視化する（「見えない救済より見える保留」という ADR-015 の思想の延長）。multimodal 読み取りへの本格対応は、当初通り実需要が発生した時点の拡張トリガーのまま据え置く。
...
## ADR-016: 個別対応にアプローチ方法選択＋人数分岐（個別/セグメント）＋ゲート整理を導入

**ステータス**: 採用（実装済み — 2026-07-11）

**背景**:
原則7（ADR-007）のセグメント方式は、実運用（会話ログ）で3つの歪みが見つかった。

1. **アプローチ方法が「フォローアップメール」に決め打ち**。`_SYSTEM_PROMPT` の個別カスタマイズ節は
   メール／架電／個別資料などの検討を一切促さず、`output_format`（コードは EMAIL/TALK_SCRIPT/
   PROPOSAL に対応済み）を案内しないため常に EMAIL へ落ちていた。
2. **アプローチ可否・到達可能総数の確認が無い**。メールならアドレス欠損者は到達不可のはずだが、
   その判定も総数提示も無いままセグメント設計・生成が進んでいた。
3. **人数に関わらず常にセグメント化**。セグメント化は本来「対象が多いときにカスタマイズ数・
   確認負荷を減らす」ための手段（原則7）だが、少人数でも例外なく軸設計→分類を強制していた。
4. **HILゲートの二重確認**。プロンプトが各ツールについて「提案」と「実行します」の宣言を
   別ターンで求めるため、1ゲートあたり2回の承認が要求され、1施策で ~7 回の「ok」が必要になり
   運用上使いづらくなっていた。

**決定**:
個別対応の進め方を**3ゲート＋人数分岐**に整理する。

1. **ゲートA（方針・可否）を新設**: run_python_code で分布を把握し、アプローチ方法の組み合わせ
   （まずはメールを推奨）と、選んだ方法での到達可否・到達可能総数（メールなら email 欠損者を
   「到達不可 N 名」と注記）を1回で提示する。ここで対象人数から**個別方式／セグメント方式**を
   決める（目安: 到達可能 15名以下 → 個別方式、それ超 → セグメント方式）。
2. **セグメント化を「多数向けの例外的手段」と明確化**し、少人数時は軸設計・分類を経由しない
   個別方式を新設する。新ツール `generate_individual_deliverables`（+ ヘルパー
   `_generate_one_individual` / `_individual_run`、`backend/agents/marketing_agent.py`）が、
   対象者ごとに1回のLLM呼び出しでプレースホルダなしの実名フル生成を行い、`run_assembly` と
   同じ `Deliverable`/`MarketingRun` 形状で保存する（既存の CSV エクスポート
   `GET /api/marketing/runs/{id}/export` を無改修で流用できる）。
3. **ゲート数は3つ（①方針・可否 ②セグメント条件＋人数〔多数時のみ〕 ③文面）に固定し、
   各ゲートは1回の応答で確認する**。多数時は `define_segment`＋`assign_segment` を連続実行して
   からゲート②を提示し、ゲート③承認後は `run_assembly` を追加確認なしで自動実行する。少数時は
   ゲート②を経由せず、ゲート③承認後の `generate_individual_deliverables` がそのまま確定処理となる。
4. `_generate_one_pattern` の生成トリガ文字列（旧: 常に「メールパターン」固定）を
   `format_label` 使用に修正し、TALK_SCRIPT/PROPOSAL 選択時の齟齬を解消する。

引き続き HIL はサーバー側の承認強制を持たず、システムプロンプトの指示＋会話ターンで薄く実現する
（[CURRENT_ISSUES.md](2026-07-02_レビュー/CURRENT_ISSUES.md) §2 が指摘するサーバー側強制の欠如は
本 ADR のスコープ外のまま将来課題とする）。

**理由**:
- ゲートAを先頭に独立させるのは、方法・可否が決まらないと後続（セグメント軸／個別方式のどちらを
  取るか）の前提が定まらないため。方法検討とセグメント設計を混ぜると、ユーザーが方法を変更したい
  ときに手戻り範囲が曖昧になる。
- 個別方式を「既存ツールの流用」ではなく専用ツールにするのは、セグメント方式のツール
  （`generate_patterns`/`run_assembly`）がプレースホルダ＋バケット単位の設計を前提にしており、
  1人ずつの実名フル生成とは責務が異なるため（1バケット=1テンプレートという不変条件を壊さない）。
- ゲート数を3つに固定し二重確認を禁止するのは、原則7の「エージェントが自律的にワークフローを
  組み立てる」思想と矛盾しない範囲で、ユーザーの承認コストを実運用に合わせて最小化するため。
  承認回数を減らすトレードオフは、3ゲートのいずれもが依然として不可逆操作
  （`run_assembly`／`generate_individual_deliverables`）の手前に置かれていることで安全側に保つ。
- メール到達不可者をコードで除外せずプロンプトの注記に留めるのは、可否判定を業務ロジックとして
  固定するとメール以外の方法（架電・個別資料等）への拡張時に判定条件が方法ごとに増えて硬直化する
  ため。現時点では run_python_code による集計・注記で十分という判断（将来複数方法を同時実行する
  段になれば、方法別の可否判定をコード側に昇格する再 ADR を検討する）。

**結果 / 将来課題**:
- `backend/agents/marketing_agent.py`: `_SYSTEM_PROMPT` の個別カスタマイズ節を全面改稿、
  `generate_individual_deliverables` ツール・`_generate_one_individual`・`_individual_run` を追加、
  `_FORMAT_LABELS` 定数を抽出して `_generate_one_pattern` と共用。
- `backend/tests/unit/test_marketing_agent.py`: `_individual_run` の正常系・欠損 Person スキップ、
  `generate_individual_deliverables` の人数ガード（空／上限超過）を追加。
- 電話番号等、メール以外の到達可否を機械的に判定するためのフィールドは `Person` に未整備
