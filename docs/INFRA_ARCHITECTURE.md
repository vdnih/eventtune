...
- バケット: `{project-id}.firebasestorage.app`（新 Firebase 既定バケット命名）
...
- リポジトリの `frontend/` を指定してデプロイ。構成は `frontend/apphosting.yaml`。
  **`firebase.json` の `hosting`（Web Frameworks）ブロックは不使用**（App Hosting に統一・ADR-012）。
- live branch（`main`）への git push で自動ビルド&デプロイ（GitHub 連携は Developer Connect で一度だけ OAuth）。
- `/api/v1/*` リクエストは `next.config.ts` の rewrite で `NEXT_PUBLIC_API_URL`（Cloud Run `mmg-api`）へ転送。
...
パッケージ管理は uv（pip 不使用）。実体は `backend/Dockerfile`。
...
> Artifact Registry へ push → `gcloud run deploy mmg-api`。詳細は ADR-012。
...
## 環境変数（Cloud Run 平文 env）

> 更新（ADR-012）: 旧版は下表を「Secret Manager 管理」としていたが、いずれも秘匿情報ではない
> （プロジェクト ID・ロケーション・Agent Engine ID・配信オリジン）。**Cloud Run の平文 env**
> として Terraform（`infra/terraform/cloud_run.tf`）が設定する。`AGENT_ENGINE_*` は
> `provision_agent_engine.py` の出力を Terraform 変数（tfvars）経由で注入。Secret Manager は
> 将来の真の秘密用に SA 権限（`secretmanager.secretAccessor`）だけ残す。
