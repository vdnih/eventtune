...
EventTune は、単なるメール生成ツールではありません。イベント（展示会・セミナー・カンファレンスなど）で
生まれた出会いを意味で統合し、根拠つきの個別フォローと振り返り分析を実行する、
イベントマーケAIエージェントです。

イベントで集めたカオスなマーケティングデータを **オントロジー** に統合し、
...
本リポジトリは、**コード**と**ドキュメント／コンテンツ**で異なるライセンスを適用します（デュアルライセンス）。

- **ソースコード**（`frontend/`, `backend/`, `infra/` 等） → **Apache License 2.0**（[`LICENSE`](LICENSE)）
- **ドキュメント／コンテンツ**（`docs/`, `sample_data/`、およびリポジトリ内のオリジナル画像・図・テキスト） →
  **Creative Commons Attribution 4.0 International（CC BY 4.0）**（[`LICENSE-CC-BY-4.0.md`](LICENSE-CC-BY-4.0.md) / [deed.ja](https://creativecommons.org/licenses/by/4.0/deed.ja)）

> **スコープ注記**：上記は本リポジトリの**オリジナル成果物**にのみ適用されます。第三者の依存ライブラリ・
> フォント・アイコン・ロゴ・他社製品の画面等、**第三者に権利がある素材はこの限りではなく、各自のライセンスに従います**。

### クレジット表示（CC BY 4.0）

CC BY 4.0 のコンテンツを利用する場合は、原作者クレジットの表示をお願いします。以下をそのままご利用いただけます：

> "EventTune" by vdnih, licensed under CC BY 4.0. Source: https://github.com/vdnih/eventtune

なお本作品は [ProtoPedia](https://protopedia.net/) に登録しており、掲載された作品情報（紹介テキスト・画像・動画等）にも
CC BY 4.0 が適用されます。
