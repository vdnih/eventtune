...
export default function ExplorerPage() {
  ...
  // データビュー内のタブ選択 ("info" | "kpi" | "costs" | "survey" | "batches")
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (evList.length > 0 && !selectedEventId) {
          setSelectedEventId(evList[0].event_id);
        }
      }
    } catch (e) {
  ...
  return (
    <div className="flex h-[calc(100vh-3.5rem)] overflow-hidden bg-gray-50 text-gray-800">
      {/* 1. 左サイドバー: イベント選択リスト */}
      <aside className="w-64 shrink-0 border-r border-gray-200 bg-white flex flex-col h-full overflow-hidden">
        <div className="px-4 py-3 border-b border-gray-200 flex items-center justify-between bg-gray-50">
          <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider flex items-center gap-1.5 font-bold">
        ...
        <div className="flex-1 overflow-y-auto px-2 py-3 space-y-1">
          {loadingEvents ? (
            ...
            <p className="text-xs text-gray-400 px-3 py-2 text-center">イベントがありません</p>
          ) : (
            events.map((ev) => (
              <button
                key={ev.event_id}
                onClick={() => setSelectedEventId(ev.event_id)}
                className={`w-full text-left rounded-lg px-3 py-2.5 transition flex flex-col gap-1 ${
                  selectedEventId === ev.event_id
                    ? "bg-brand-50 text-brand-900 border border-brand-200 shadow-sm font-semibold"
                    : "hover:bg-gray-50 border border-transparent text-gray-600 hover:text-gray-800"
                }`}
              >
                <span className="text-xs font-bold leading-tight line-clamp-2">{ev.name}</span>
                <div className="flex items-center justify-between w-full mt-1">
                  <span className="text-[10px] text-gray-400 font-mono">{ev.event_date}</span>
                  <span
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden bg-white">
        {!selectedEventId ? (
          <div className="flex-1 flex flex-col items-center justify-center text-gray-400 gap-2">
            <Database className="w-12 h-12 stroke-[1.25] text-gray-300" />
            <p className="text-sm">左側のリストからイベントを選択してください。</p>
          </div>
      ...
      {/* 3. 詳細判定根拠ドロワー (スライドインサイドパネル - Auditable AIの可視化) */}
      {activeDrawerReport && (
